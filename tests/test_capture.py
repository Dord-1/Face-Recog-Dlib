...
import capture
from config import DETECT_SCALE, MAX_YAW_RATIO, MIN_FACE_WIDTH
...
def patch_landmarks(monkeypatch, landmarks):
    monkeypatch.setattr(capture.face_recognition, 'face_landmarks', lambda *a, **k: landmarks)
...
class TestCheckFrame:
    ...
    def test_too_dark(self, monkeypatch):
        ...
        patch_landmarks(monkeypatch, frontal_landmarks())
        valid, message, box = capture.check_frame((frame() * 0.2).astype(np.uint8))
        assert not valid and 'toi' in message and box is not None

    def test_too_bright(self, monkeypatch):
        patch_locations(monkeypatch, [location_with_width(200)])
        patch_landmarks(monkeypatch, frontal_landmarks())
        valid, message, _ = capture.check_frame(np.full((480, 640, 3), 245, dtype=np.uint8))
    ...
    def test_blurry(self, monkeypatch):
        ...
        assert capture.check_frame(sharp_frame)[0]                       # bản nét thì hợp lệ
        valid, message, _ = capture.check_frame(cv2.GaussianBlur(sharp_frame, (0, 0), 4))
        assert not valid and 'mo' in message

    def test_face_not_frontal(self, monkeypatch):
        patch_locations(monkeypatch, [location_with_width(200)])
        patch_landmarks(monkeypatch, turned_landmarks())
        valid, message, _ = capture.check_frame(frame())
    ...
    def test_missing_landmarks_does_not_block_capture(self, monkeypatch):
        ...
        valid, _, _ = capture.check_frame(frame())
        assert valid

    def test_landmarks_not_computed_when_earlier_check_fails(self, monkeypatch):
        patch_locations(monkeypatch, [location_with_width(200)])
        monkeypatch.setattr(capture.face_recognition, 'face_landmarks',
                            lambda *a, **k: pytest.fail('không được tính landmarks khi đã thất bại'))
        valid, _, _ = capture.check_frame((frame() * 0.2).astype(np.uint8))
        assert not valid
...
class TestQualityMetrics:
    ...
    def test_sharpness_drops_with_blur(self):
        sharp = self.textured()
        assert capture.sharpness(cv2.GaussianBlur(sharp, (0, 0), 3)) < capture.sharpness(sharp) / 3

    def test_sharpness_is_invariant_to_darkening(self):
        img = self.textured()
        darker = (img * 0.5).astype(np.uint8)
        assert capture.sharpness(darker) == pytest.approx(capture.sharpness(img), rel=0.15)

    def test_sharpness_of_flat_image_is_zero(self):
        assert capture.sharpness(np.full((100, 100), 120, dtype=np.uint8)) == 0.0
    ...
    def test_nearly_flat_image_is_not_scored_sharp_from_quantization_noise(self):
        ...
        assert capture.sharpness(nearly_flat) < 0.1

    def test_brightness_is_mean_gray(self):
        assert capture.brightness(np.full((100, 100), 77, dtype=np.uint8)) == pytest.approx(77.0)

    def test_yaw_ratio_frontal_is_about_one(self):
        assert capture.yaw_ratio(frontal_landmarks()[0]) == pytest.approx(1.0, abs=0.1)

    def test_yaw_ratio_grows_when_turned(self):
        assert capture.yaw_ratio(turned_landmarks()[0]) > MAX_YAW_RATIO
    ...
    def test_yaw_ratio_is_symmetric_and_at_least_one(self):
        ...
        assert capture.yaw_ratio(lm) == pytest.approx(capture.yaw_ratio(mirrored))
        assert capture.yaw_ratio(lm) >= 1.0
