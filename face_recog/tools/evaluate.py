...
from face_recog.config import DETECT_DIR, DETECT_SCALE, INV_SCALE
from face_recog.geometry import largest_face, scale_locations
from face_recog.known_faces import list_image_files, load_known_faces, person_name
...
def list_eval_images(eval_dir):
    ...
    for label in sorted(os.listdir(eval_dir)):
        ...
        if not os.path.isdir(folder):
            continue
        images = [os.path.join(folder, f) for f in list_image_files(folder)]
        if images:
            found[label] = images
    return found
...
def encode_test_image(frame, mode):
    ...
    if not small_locations:
        return None
    largest = largest_face(small_locations)
